...
// Higher baseline resolution so a full-HD browser window is shown close to
// native size under noVNC's resize=scale (see public/app.js) instead of the
// old 1280x800 framebuffer being stretched up, which is what caused the blur.
// NOTE: we deliberately do NOT use resize=remote/-xrandr dynamic resizing —
// there is no window manager in this container, so Chromium's already-open
// kiosk window would not shrink/grow to match a resized Xvfb screen, which
// cropped and hid part of the page for anyone whose viewport did not match
// the resized resolution exactly.
...
async function boot() {
  ...
  if (exists('/usr/bin/x11vnc') && !vnc) {
    vnc = spawnLogged('x11vnc', ['-display', DISPLAY, '-forever', '-shared', '-nopw', '-localhost', '-rfbport', String(VNC_PORT), '-quiet', '-nocursor']);
  }
